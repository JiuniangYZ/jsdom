...
const { getStyle } = require('../../../../../mutationHandler/utils');
...
function setupWindow(windowInstance, { runScripts }) {
  if (runScripts === "outside-only" || runScripts === "dangerously") {
    throw new Error('donot support this runScripts mode');
    // // 不存在这个模式 不要想了
    ...
    console.log('not setting up window')
...
function Window(options) {
  ...
  // if (vm.isContext(window)) { // 不可能的 别想了
    ...
    function task() {
      ...
      try {
        if (typeof handler === "function") {
          ...
          throw new Error('dangerous not supported')
  ...
  this.getComputedStyle = function (elt, pseudoElt = undefined) {
    console.log('debug_get_compute_style==>',elt)
    if(!elt.shadow_id){
      throw new Error('cannot get shadowID for elt')
    }
    const res = getStyle(elt,true);
    console.log('get sth ==> ',res);
    try{
