...
const { submitELmutation } = require('../../../../../../mutationHandler/utils')
...
exports.install = (globalObject, globalNames) => {
  ...
  class Node extends globalObject.EventTarget {
    ...
    get shadow_id() {
      return this[implSymbol]["_shadow_id"];
    }

    set shadow_id(id) {
      console.log(`element shadow_id set as ${id}`);
      this[implSymbol]["_shadow_id"] = Number(id);
    }
    //TODO: 帮助确定是否增加/消除了某event监听状态的工具方法
    increaseEventHandlerCount(event){
      const counterMap = this[implSymbol].eventHandlerCounter;
      const r = counterMap.get(event);
      if(typeof r === 'undefined'){
        counterMap.set(event, 1);
      }else{
        counterMap.set(event, r+1);
      }
      if(!r){
        return true; // true 说明状态变化了
      }
      return false;
    }

    //TODO: 帮助确定是否增加/消除了某event监听状态的工具方法
    decreaseEventHandlerCount(event){
      const counterMap = this[implSymbol].eventHandlerCounter;
      const r = counterMap.get(event);
      if(!r) { // r===0 or r=>undefined
        return false;
    ...
    //TODO 劫持了add/remove eventlistener
    addEventListener(...args){
      super.addEventListener(...args);
      submitELmutation(this, args[0], false);
    }
